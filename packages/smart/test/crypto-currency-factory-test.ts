...
describe("CryptoCurrencyFactory", function () {
  ...
  [
    ...
    it(`fetcher initial {offset=${offset},bundle=${bundle}}`, async () => {
      const { factoryBundle, markets } = await fetchInitialCryptoCurrency(
        fetcher,
        marketFactory,
        ammFactory,
        masterChef,
    ...
    it(`fetcher dynamic {offset=${offset},bundle=${bundle}}`, async () => {
      const { markets } = await fetchDynamicCryptoCurrency(fetcher, marketFactory, ammFactory, offset, bundle);
