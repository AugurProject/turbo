...
import {
  ...
  FeePot,
  CryptoPriceFetcher,
  CryptoPriceFetcher__factory,
  FakePriceFeed,
  OwnedERC20__factory,
} from "../typechain";
import {
  calcShareFactor,
  fetchDynamicCryptoPrice,
  fetchInitialCryptoPrice,
  PRICE_FEEDS,
  NULL_ADDRESS,
...
describe.only("CryptoPriceFactory", function () {
  enum CoinIndex {
    None,
    BTC,
    ETH,
  ...
  let marketFactory: CryptoPriceMarketFactoryV3;
  ...
  before("other contracts", async () => {
    marketFactory = (await ethers.getContract("CryptoPriceMarketFactoryV3")) as CryptoPriceMarketFactoryV3;
  ...
  it("BTC price market is correct", async () => {
    ...
    expect(marketDetails.creationPrice, "creationPrice").to.equal(btcPrice);
  ...
  it("NewPrice log, by resolution time", async () => {
    const filter = marketFactory.filters.PriceUpdate(null, nextResolutionTime, null, null);
    const logs = await marketFactory.queryFilter(filter);
  ...
  it("fetcher deploys", async () => {
    fetcher = (await ethers.getContract("CryptoPriceFetcher")) as CryptoPriceFetcher;

    expect(await fetcher.marketType()).to.equal("CryptoPrice");
  ...
  [
    ...
    it(`fetcher initial {offset=${offset},bundle=${bundle}}`, async () => {
      const { factoryBundle, markets } = await fetchInitialCryptoPrice(
        fetcher,
        marketFactory,
        ammFactory,
        offset,
    ...
    it(`fetcher dynamic {offset=${offset},bundle=${bundle}}`, async () => {
      const { markets } = await fetchDynamicCryptoPrice(fetcher, marketFactory, ammFactory, offset, bundle);
