...
task("multicalls", "get total supply for bpool contract")
  ...
  .setAction(async ({ marketfactoryaddress, marketindex }: { marketfactoryaddress: string, marketindex: number }, hre) => {
    ...
    const [contract] = MarketFactories.filter((f) => f.marketFactory.address.toLowerCase() === marketfactoryaddress.toLowerCase()).map(
      (f) => f.marketFactory
    );
    ...
    const addressMapping = addresses[networkId]?.marketFactories.find(m => m.address.toLowerCase() === marketfactoryaddress.toLowerCase());
