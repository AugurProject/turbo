...
task("cannedMarkets", "creates canned markets")
    .setAction(async (args, hre) => {
        ...
        const extraInfo = "{title: 'Here is a Categorical Market'}";
        const prices: BigNumberish[] = [0, 2000];
        const marketType = MarketTypes.CATEGORICAL;
        const arbiterConfiguration = await TrustedArbiter.encodeConfiguration(startTime, duration, extraInfo, prices, marketType);
