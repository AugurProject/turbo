...
contract CryptoPriceMarketFactoryV3 is AbstractMarketFactoryV3, CalculateLinesToBPoolOdds, Versioned, ManagedByLink {
    using SafeMathUint256 for uint256;
    using SafeMathInt256 for int256;

    event CoinAdded(uint256 indexed id, string name);
    event PriceUpdate(uint256 indexed coinIndex, uint256 indexed resolutionTime, uint256 market, uint256 price);
    ...
    function createMarket(
        ...
        (, uint256 _newPrice) = getLatestPrice(_coin);
        ...
        marketDetails[_marketId] = MarketDetails(_coinIndex, _newPrice, 0, _resolutionTime);
        _coin.currentMarket = _marketId;
        _coin.price = _newPrice;
        emit PriceUpdate(_coinIndex, _resolutionTime, _marketId, _newPrice);
    }
    ...
    function resolveMarket(Coin storage _coin, uint80 _roundId) internal {
        ...
        (uint256 _fullPrice, uint256 _newPrice) = getSpecificPrice(_coin, _roundId, _resolutionTime);
        ...
        marketDetails[_coin.currentMarket].resolutionPrice = _fullPrice;
        _coin.currentMarket = 0;
        _coin.price = 0;
    }

    function getLatestPrice(Coin storage _coin) internal view returns (uint256 _fullPrice, uint256 _truncatedPrice) {
        ...
        _truncatedPrice = calcTruncatedPrice(_coin, _fullPrice);
    }

    // Get price at a specific round, but fail if it isn't after a specific time.
    function getSpecificPrice(
        Coin storage _coin,
        uint80 _roundId,
        uint256 _resolutionTime
    ) internal view returns (uint256 _fullPrice, uint256 _truncatedPrice) {
        (, int256 _rawPrice, , uint256 _updatedAt, ) = _coin.priceFeed.getRoundData(_roundId);
        require(_rawPrice >= 0, "Price from feed is negative");
        require(_updatedAt >= _resolutionTime, "Price hasn't been updated yet");
    ...
    function calcTruncatedPrice(Coin storage _coin, uint256 _fullPrice)
        internal
        view
        returns (uint256 _truncatedPrice)
    {
    ...
    function makeCoin(
        ...
        _coin = Coin(_name, _priceFeed, 0, _imprecision, 0);
    }
