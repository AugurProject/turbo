...
import * as MMAFetcher from "./fetch-mma";
...
export const fillGraphMarketsData = async (
  ...
  let config = getMarketFactoryData("0x39Fb172fCBFBf8E594cA15a31B3bBd88E50C9B68");
  const mma = await MMAFetcher.fetchContractData(config, provider, account);
  console.log("mma", mma);
  config = getMarketFactoryData("0x5524863C74404FC2776Bacb633F3D91fF8143df8");
  const nbamlb = await SportFetcher.fetchContractData(config, provider, account);
  console.log("nba mlb", nbamlb);
