...
export const getOutcomeName = (index: number, sportId: string, homeTeam: string, awayTeam: string, sportsMarketType: number, line: string) => {
...
export const getMarketTitle = (sportId: string, homeTeam: string, awayTeam: string, sportsMarketType: number, line: string): {title: string, description: string} => {
    let title = "";
    let description = "";
    if (sportsMarketType === 0) {
      // head to head (money line)
